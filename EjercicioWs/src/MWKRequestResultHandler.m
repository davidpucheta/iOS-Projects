...
@implementation MWKRequestResultHandler
...
-(id)init:(int)version {
    if ((self=[super init])) {
        ...
        if(!classNames)
        {
            classNames=[NSDictionary dictionary];
...
-(id)createObject: (DDXMLElement*) node type:(Class) type
{
    ...
    DDXMLNode* nilAttr=[MWKHelper getAttribute:node name:@"nil" url:XSI];
    if(nilAttr!=nil && [[nilAttr stringValue]boolValue])
    {
        return nil;
    }

    DDXMLNode* typeAttr=[MWKHelper getAttribute:node name:@"type" url:XSI];
    if(typeAttr !=nil)
    {
    ...
    DDXMLNode* hrefAttr=[MWKHelper getAttribute:node name:@"href" url:@""];
    if(hrefAttr==nil)
    {
        hrefAttr=[MWKHelper getAttribute:node name:@"ref" url:@""];
    }
    if(hrefAttr!=nil)
    {
...
-(id) createInstance:(Class) type node: (DDXMLNode*)node request :(MWKRequestResultHandler *)request
{
    ...
    id (*func)(id, SEL, DDXMLNode*, MWKRequestResultHandler *) = (void *)imp;
...
-(void)setResponse:(NSData *)responseData response:(NSURLResponse*) response
{
    ...
    OutputBody=[MWKHelper getNode:__root  name:@"Body" URI:envNS];
    OutputHeader=[MWKHelper getNode:__root  name:@"Header" URI:envNS];
    DDXMLElement* fault=[MWKHelper getNode:OutputBody  name:@"Fault" URI:envNS];
    if(fault!=nil)
    {
        DDXMLElement* faultString=[MWKHelper getNode:fault name:@"faultstring"];
        if(faultString == nil)
        {
            faultString=[MWKHelper getNode:fault name:@"Reason"];
            if(faultString!=nil)
            {
                faultString = [MWKHelper getNode:faultString name:@"Text"];
            }
        }
        id faultObj=nil;
        DDXMLElement* faultDetail=[MWKHelper getNode:fault name:@"detail"];
        if(faultDetail!=nil)
        {
        ...
        OutputFault=[[MWKSoapError alloc] initWithDetails:[faultString stringValue] details:faultObj];
    }
}
